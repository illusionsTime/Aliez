...
如果是skiplist编码的有序集合对象使用zset作为底层实现，一个zset界沟通是包含一个字段和一个跳跃表
```c
typedef struct zset{
	zskiplist *zsl;
	dict *dict;
...
### 一面
* go和java的区别？
  1. java允许多态，go没有，同样的java支持函数重载，golang中函数必须具有唯一性
...
* mysql事务隔离级别？
  1. READ UNCOMMITTED 读未提交 该隔离级别的事务会读到其它未提交事务的数据，此现象也称之为 脏读 。
  2. READ COMMITTED 一个事务可以读取另一个已提交的事务，多次读取会造成不一样的结果，此现象称为不可重复读问题
  3. REPEATABLE READ InnoDB支持的隔离级别，但是使用NEXT-key算法避免了幻读的产生 可重复读
